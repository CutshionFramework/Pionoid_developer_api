...
def move_robot(robot: core_robot, joint_positions):
...
if __name__ == "__main__":
    ...
    ur_robot = URRobot("192.168.88.128")
    initialize_robot(ur_robot)
    move_robot(ur_robot, [1, 0, 0, 0, 0, 0])
    print_robot_status(ur_robot)
    ur_robot.logout()

    # Example with JakaRobot
    jaka_robot = JakaRobot("192.168.0.128")
    initialize_robot(jaka_robot)
    move_robot(jaka_robot, [1, 0, 0, 0, 0, 0])
